...
Comment: Solves the system FORWARD in time taking the last values of the backwards result as INITIAL CONDITIONS.
...
class DarkM:
    ...
    def solverBG(self):
        ...
        y_result = self.ABM4(self.RHSBG, y0, self.t)
        #y_result = self.rk4(self.RHS, y0, self.t)
        return y_result

    # Perturbations: Condiciones iniciales para las variables, valores que se tienen al dia de hoy. 
    def solverPB(self):
        z0, z2, z4, z5, z6, z7, z8 = self.solverBG().T
        length = self.NP - 1

        y0 = np.array([z0[length],      # Small initial kinetic energy to avoid 1/x divergence
                       z2[length],      # u Initial Condition
                       z4[length],      # z Initial Condition
                       z5[length],      # nu Initial Condition
                       z6[length],      # b Initial Condition
                       z7[length],      # l Initial Condition
                       z8[length],      # s Initial Condition
                       1.0e-33,           # l1 Initial Condition
                       1.0e-33,           # l2 Initial Condition
                       1.0e-33,           # z1 Initial Condition
                       1.0e-33])          # z2 Initial Condition   

        y_result = self.ABM4(self.RHSPB, y0, self.t[::-1])
        #y_result = self.rk4(self.RHS, y0, self.t)
        return y_result
...
#Abajo se tiene la funcion que va a imprimir las graficas.
    def plot(self):
        ...
        z0, z2, z4, z5, z6, z7, z8, z9, z10, z11, z12 = self.solverPB().T
        ...
        for t, aux0, aux2, aux4, aux5, aux6, aux7, aux8, aux9, aux10, aux11, aux12  in zip(self.t[::-1], z0, z2, z4, z5, z6, z7, z8, z9, z10, z11, z12):
