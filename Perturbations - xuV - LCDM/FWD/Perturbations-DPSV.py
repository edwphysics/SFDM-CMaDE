...
Cosmological Constant & Scalar Field Dark Matter: Perturbations.

Combination of the SFDM and LCDM model equations. Perturbation equations with T<<Tc.
...
Comment: First, solve the background equations backwards in time. Then, it uses the result to solve the perturbation equations FORWARD in time.
...
class DarkM:
    def __init__(self, mquin=1, name='LO'):
        ...
        # Parameter Densities Measured Today Planck2018
        self.OmegaDM = 0.11907/self.h**2
        self.OmegaB  = 0.02234/self.h**2
        self.OmegaNu = 2.0e-5
        self.OmegaR  = 4.0e-5
        self.OmegaL  = 1.0 - self.OmegaDM - self.OmegaB - self.OmegaR - self.OmegaNu 
    ...
    def solverBG(self):
        y0 = np.array([np.sqrt(1.0e-9),                     # Small initial kinetic energy to avoid 1/x divergence
                       np.sqrt(self.OmegaDM - 1.0e-9),      # u Initial Condition
                       np.sqrt(self.OmegaR),                # z Initial Condition
                       np.sqrt(self.OmegaNu),               # nu Initial Condition
                       np.sqrt(self.OmegaB),                # b Initial Condition
                       np.sqrt(self.OmegaL),                # l Initial Condition
    ...
    def RHSBG(self, t, y, z, args={}):
        ...
        #Pe = 1 + wef
        return np.array([-3.0* x0 - x8*x2 + 1.5* x0*Pe,
                         x0*x8 + 1.5*x2*Pe,
                         1.5* x4* (Pe - CTer),
                         1.5* x5* (Pe - CTer),
                         1.5* x6* (Pe - 1.0),
